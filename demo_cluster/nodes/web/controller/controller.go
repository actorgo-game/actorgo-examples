...
import (
	...
	"github.com/actorgo-game/examples/demo_cluster/nodes/web/sdk"
)
...
// index h5客户端
func (p *Controller) index(c *cgin.Context) {
	c.HTML200("index.html")
}

// hello 输出json示例
// http://127.0.0.1/hello
func (p *Controller) hello(c *cgin.Context) {
	// 输出json
...
func (p *Controller) register(c *cgin.Context) {
...
func (p *Controller) login(c *cgin.Context) {
	pid := c.GetInt32("pid", 0, true)

	if pid < 1 {
		clogger.Warn("if pid < 1 {. params=%s", c.GetParams())
	...
	if config == nil {
		clogger.Warn("if platformConfig == nil {. params=%s", c.GetParams())
	...
	if err != nil {
		clogger.Warn("[pid = %d] get invoke error. params=%s", pid, c.GetParams())
		code.RenderResult(c, code.PIDError)
		return
	}

	params := c.GetParams(true)
	params["pid"] = cstring.ToString(pid)

	// invoke login
	sdkInvoke.Login(config, params, func(statusCode int32, result sdk.Params, error ...error) {
		if code.IsFail(statusCode) {
			clogger.Warn("login validate fail. code = %d, params = %s", statusCode, c.GetParams())
			if len(error) > 0 {
				clogger.Warn("code = %d, error = %s", statusCode, error[0])
			}
		...
		if result == nil {
			clogger.Warn("callback result map is nil. params= %s", c.GetParams())
			code.RenderResult(c, code.LoginError)
			return
		}
		...
		if found == false {
			clogger.Warn("callback result map not found `open_id`. result = %s", result)
			code.RenderResult(c, code.LoginError)
			return
		}
...
func (p *Controller) serverList(c *cgin.Context) {
	pid := c.GetInt32("pid", 2126001)

	if pid < 1 {
		clogger.Warn("if pid < 1 {. params=%v", c.GetParams())
		code.RenderResult(c, code.PIDError)
		return
	}
