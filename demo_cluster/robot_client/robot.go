...
func (p *Robot) GetToken(url string, pid, userName, password string) error {
	...
	jsonBytes, _, err := chttp.GET(requestURL, map[string]string{
		"pid":      pid,      //sdk包id
	...
	if code.IsFail(rsp.Code) {
		return cerror.Errorf("get Token fail. [message = %s]", rsp.Message)
	}

	// 获取token值
	p.Token = rsp.Data.(string)
	p.TagName = fmt.Sprintf("%s_%s", pid, userName)
	p.StartTime = ctime.Now()
...
func (p *Robot) UserLogin(serverId int32) error {
	...
	p.Debug("[%s] [UserLogin] response = %+v", p.TagName, rsp)
	return nil
}
...
func (p *Robot) PlayerSelect() error {
	...
	if len(rsp.List) < 1 {
		p.Debug("[%s] not found player list.", p.TagName)
		return nil
	}
	...
	p.Debug("[%s] [PlayerSelect] response PlayerID = %d,PlayerName = %s", p.TagName, p.PlayerId, p.PlayerName)
...
func (p *Robot) ActorCreate() error {
	if p.PlayerId > 0 {
		p.Debug("[%s] deny create actor", p.TagName)
		return nil
	}
	...
	p.Debug("[%s] [ActorCreate] PlayerID = %d,ActorName = %s", p.TagName, p.PlayerId, p.PlayerName)
...
func (p *Robot) ActorEnter() error {
	...
	p.Debug("[%s] [ActorEnter] response PlayerID = %d,ActorName = %s", p.TagName, p.PlayerId, p.PlayerName)
	return nil
}
...
func (p *Robot) Debugf(args ...interface{}) {
	if p.PrintLog {
		clogger.Debug("", args...)
	}
...
func (p *Robot) Debug(template string, args ...interface{}) {
	if p.PrintLog {
		clogger.Debug(template, args...)
	}
}
